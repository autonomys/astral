...
export const MobileCard: FC<Props> = ({ header, body, id, children }) => {
  return (
    <div className="mb-6 w-full rounded-lg bg-white px-4 py-7 font-['Montserrat'] dark:bg-gradient-to-r dark:from-gradientFrom dark:via-gradientVia dark:to-gradientTo">
      ...
      <div className='w-full divide-y divide-gray-200 dark:divide-white/20'>
        {body.map(({ name, value }, index) => (
          <div className='flex items-center justify-between py-2' key={`${id}-${index}`}>
            <div className='text-xs text-purpleShade  dark:text-white/75'>{name}</div>
