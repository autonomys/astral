...
function AccountListDropdown() {
  const { actingAccount, sessionSubspaceAccount, accounts, changeAccount, disconnectWallet } =
    useWallet()
    ...
    <Listbox value={actingAccount} onChange={changeAccount}>
      <div className='relative'>
        ...
        >
          <div className='flex items-center justify-center'>
            ...
            <span className='ml-2 hidden w-5 truncate text-sm sm:block md:w-full '>
              {sessionSubspaceAccount ? shortString(sessionSubspaceAccount) : 'Select Account'}
            </span>
        ...
        <Transition
          ...
          leaveTo='opacity-0'
        >
          <Listbox.Options className='absolute right-0 z-30 mt-1 max-h-80 w-full overflow-auto rounded-md bg-white py-2 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none dark:bg-blueAccent dark:text-white sm:text-sm'>
            {walletList}
            <button
