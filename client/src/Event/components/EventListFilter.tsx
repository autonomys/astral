...
const EventListFilter: FC<Props> = ({ filters, setFilters }) => {
  ...
  const handleAccountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setWhere((prev) => ({ ...prev, block: { author: { id_eq: e.target.value } } }))
  }
      ...
      <div>
        <div className='grid grid-cols-2 md:grid-cols-5 gap-4 mt-4 items-end'>
          ...
          <div className='md:col-span-3 flex flex-col gap-3'>
            <div className='text-[13px] font-semibold text-[#282929] dark:text-white'>Engine</div>
            <input
              type='text'
              placeholder='Optional'
              onChange={handleAccountChange}
              value={where?.block?.author?.id_eq || ''}
              className='px-4 py-3 w-full rounded-[42px] bg-white dark:bg-[#1E254E] dark:text-white border-transparent focus:border-gray-500 focus:bg-white focus:ring-0 text-sm'
            />
          </div>
          ...
          <button
            ...
            className='md:col-span-3 md:justify-self-end px-[33px] py-[13px] bg-white dark:bg-[#1E254E] hover:bg-gray-200 text-gray-800 dark:text-white text-sm font-medium rounded-[20px]'
          >
            Filter
          </button>
        </div>
