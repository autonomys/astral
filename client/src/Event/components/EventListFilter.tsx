...
type Props = {
  ...
  timeDimension: 'date' | 'block'
  setTimeDimension: React.Dispatch<React.SetStateAction<'date' | 'block'>>
  filters: EventWhereInput
  setFilters: React.Dispatch<React.SetStateAction<EventWhereInput>>
}

const EventListFilter: FC<Props> = ({
  title,
  setFilters,
  filters,
  timeDimension,
  ...
  const handleModuleChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    setWhere((prev) => ({
      ...prev,
      name_containsInsensitive: e.target.value,
    }))
  }
            ...
            <div className='grid grid-cols-2 md:grid-cols-5 gap-4 mt-4 items-end'>
              ...
              <div className='md:col-span-3 flex flex-col gap-3'>
                ...
                <input
                  ...
                  value={where?.extrinsic?.signer?.id_eq || ''}
                  className='px-4 py-3 w-full rounded-[42px] bg-white dark:bg-[#1E254E] dark:text-white border-transparent focus:border-gray-500 focus:bg-white focus:ring-0 text-sm'
                />
              </div>
              <div className='flex flex-col gap-3'>
