...
const MobileCard: FC<Props> = ({ header, body, id, children }) => {
  return (
    <div className="w-full bg-white rounded-[20px] dark:bg-gradient-to-r dark:from-[#4141B3] dark:via-[#6B5ACF] dark:to-[#896BD2] rounded-lg py-7 px-4 font-['Montserrat'] mb-6">
      ...
      <div className='w-full divide-y divide-gray-200 dark:divide-white/20'>
        {body.map(({ name, value }, index) => (
          <div className='flex justify-between items-center py-2' key={`${id}-${value}-${index}`}>
            ...
            <div className='text-[#241235] text-xs dark:text-white'>{value}</div>
          </div>
        ))}
      </div>
      {children}
