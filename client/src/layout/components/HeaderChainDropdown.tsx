    ...
    <Listbox value={selected} onChange={setSelected}>
      <div className='relative'>
        ...
        <Transition
          ...
          leaveTo='opacity-0'
        >
          <Listbox.Options className='absolute mt-1 max-h-60 w-auto md:w-full overflow-auto rounded-md bg-white py-2 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm'>
            {chains.map((chain, chainIdx) => (
              <Listbox.Option
                key={chainIdx}
                className={({ active }) =>
                  `relative cursor-default select-none py-2 pl-4 text-gray-900 md:pl-10 pr-4 ${
                    active && 'bg-gray-100'
                  }`
                }
                value={chain}
