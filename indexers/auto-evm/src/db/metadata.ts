...
export const getMetadata = async () =>
  await sql`SELECT * FROM domain_auto_evm._metadata`;
...
export const updateMetadata = async (
  ...
  await (sqlClient || sql)`
    INSERT INTO domain_auto_evm._metadata (key, value, "createdAt", "updatedAt")
    VALUES (${key}::text, 
            ${value}::jsonb,
            NOW(),
            NOW())
    ...
    DO UPDATE SET 
      value = ${value}::jsonb,
      "updatedAt" = NOW()
    WHERE domain_auto_evm._metadata.key = ${key}::text
  `;
