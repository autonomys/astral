...
const processBatchTasks = async (tasks: AccountProcessingTask[]): Promise<number> => {
  ...
  const validTasks = tasks.filter(task => {
    // Skip future blocks (blocks ahead of current chain height)
    if (task.blockHeight > currentChainHeight) {
      console.warn(`Worker: Skipping future block task for block ${task.blockHeight} (current chain height: ${currentChainHeight})`);
      return false;
    }
    ...
    try {
      ...
      if (error instanceof Error && 
          (error.message.includes('timeout') || 
           ...
           (error as any).code === '25P02')) { // PostgreSQL transaction aborted error code
