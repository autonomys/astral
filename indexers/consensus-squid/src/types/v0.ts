...
export const Transfer: sts.Type<Transfer> = sts.struct(() => {
  return {
    amount: sts.bigint(),
    sender: Location,
    receiver: Location,
...
export const BlockMessages: sts.Type<BlockMessages> = sts.struct(() => {
  return {
    outbox: sts.array(() =>
      sts.tuple(() => [ChainId, sts.tuple(() => [sts.bigint(), sts.bigint()]), MessageWeightTag]),
    ),
...
export type ProtocolMessageRequest =
  | ProtocolMessageRequest_ChannelClose
  | ProtocolMessageRequest_ChannelOpen
...
export type DomainsCall =
  ...
  | DomainsCall_switch_domain
  | DomainsCall_unlock_funds
  | DomainsCall_unlock_operator
