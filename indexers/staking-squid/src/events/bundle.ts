...
export function processBundleStoredEvent(
  ...
  const sealedHeader = extrinsic.call?.args.opaqueBundle.sealedHeader as SealedBundleHeader
  ...
  const totalRejectedTransfersClaimed = transfers.rejectedTransfersClaimed.reduce(
    (acc, [, amount]) => acc + BigInt(amount),
    BigInt(0),
  )
  const rejectedTransfersClaimedCount = transfers.rejectedTransfersClaimed.length
  ...
  const lastBlockBundleIndex = cache.internalKeyStore.get(keyIdLastBlockBundleIndex)
  const blockBundleIndex = lastBlockBundleIndex ? parseInt(lastBlockBundleIndex) + 1 : 0
  cache.internalKeyStore.set(keyIdLastBlockBundleIndex, blockBundleIndex.toString())

  const domainEpoch = getOrCreateDomainEpoch(cache, block, domainId, domain.completedEpoch, {
  ...
  domainEpoch.blockCount = domainEpoch.blockNumberEnd - domainEpoch.blockNumberStart + 1
  ...
  let bundle = cache.bundles.get(bundleUID(domainId, domainBlockHash, blockBundleIndex))
  let domainBlock = cache.domainBlocks.get(blockUID(domainId, domainBlockNumber))
  if (!domainBlock) {
    domainBlock = createDomainBlock(block, domainId, domainBlockNumber, domainBlockHash, {
  ...
  if (!bundle) {
    bundle = createBundle(domain.id, domainBlock.id, domainBlockHash, blockBundleIndex, {
