import { operators as getOperators, parseDeposit, parseWithdrawal } from '@autonomys/auto-consensus'
...
export async function processEpochTransitionEvent(
  ...
  const withdrawals = await Promise.all(
    operatorsAll.map((o) => apiAt.query.domains.withdrawals.entries(o.operatorId)),
  ...
  for (const operator of allOperators) {
    ...
    op.currentSharePrice =
      op.currentTotalShares > BigInt(0)
        ? (op.currentTotalStake * SHARES_CALCULATION_MULTIPLIER) / op.currentTotalShares
        : BigInt(0)
    ...
    try {
      ...
      if (
        ...
        _status.deregistered.unlockAtConfirmedDomainBlockNumber <= domain.lastDomainBlockNumber
      ) {
        ...
        Array.from(cache.nominators.values())
          ...
          .forEach((n) => {
            ...
            n.pendingAction = NominatorPendingAction.PENDING_DEREGISTRATION_LOCK
            n.updatedAt = currentBlockNumber
            cache.nominators.set(n.id, n)
          })

        Array.from(cache.withdrawals.values())
          .filter((w) => w.status === WithdrawalStatus.PENDING_LOCK && w.domainId === domain.id)
          .forEach((w) => {
      ...
      console.error('Error parsing operator status in processEpochTransitionEvent', e)
    }
  }
  ...
  allOperators.forEach((o) => {
    ...
    domain.accumulatedEpochStorageFeeDeposit += o.operatorDetails.totalStorageFeeDeposit
  })
  ...
  Array.from(cache.operators.values())
    .filter((o) => o.status === OperatorStatus.REGISTERED && o.domainId === domain.id)
    .forEach((o) => {
      o.activeEpochCount++
  ...
  Array.from(cache.operators.values())
    .filter((o) => o.status === OperatorStatus.PENDING && o.domainId === domain.id)
    .forEach((o) => {
  ...
  Array.from(cache.nominators.values())
    .filter((n) => n.status === NominatorStatus.PENDING && n.domainId === domain.id)
    .forEach((n) => {
  ...
  Array.from(cache.deposits.values())
    .filter((d) => d.status === DepositStatus.PENDING && d.domainId === domain.id)
    .forEach((d) => {
  ...
  const operators = Array.from(cache.operators.values()).filter(
    (o) =>
      ...
      (o.status === OperatorStatus.REGISTERED || o.status === OperatorStatus.DEREGISTERED),
  )

  for (const operator of operators) {
    const statsPerOperator = createStatsPerOperator(cache, block, domain, operator)
    ...
    for (const nominator of nominators) {
      const statsPerNominator = createStatsPerNominator(cache, block, domain, operator, nominator)
  ...
  const allDeposits = deposits.filter((o) => operatorsIds.includes(o.operatorId.toString()))
  ...
  for (const deposit of allDeposits) {
    ...
    nominator.knownStorageFeeDeposit = deposit.known.storageFeeDeposit ?? BigInt(0)
    nominator.pendingAmount = deposit.pending?.amount ?? BigInt(0)
    nominator.pendingStorageFeeDeposit = deposit.pending?.storageFeeDeposit ?? BigInt(0)
    nominator.pendingEffectiveDomainEpoch = deposit.pending?.effectiveDomainEpoch ?? 0

    nominator.currentTotalStake = nominator.knownShares * operator.currentSharePrice
    ...
    nominator.accumulatedEpochStorageFeeDeposit += nominator.currentStorageFeeDeposit
    ...
    account.currentSharePrice =
      account.currentTotalStake > BigInt(0)
        ? (account.currentTotalStake * SHARES_CALCULATION_MULTIPLIER) / account.currentTotalShares
        : BigInt(0)

    account.accumulatedEpochStake += nominator.currentTotalStake
    account.accumulatedEpochStorageFeeDeposit += nominator.currentStorageFeeDeposit
  ...
  const allWithdrawals = withdrawals.filter((o) => operatorsIds.includes(o.operatorId.toString()))
  ...
  for (const withdrawal of allWithdrawals) {
    ...
    nominator.totalWithdrawalAmounts = withdrawal.totalWithdrawalAmount ?? BigInt(0)
    nominator.totalStorageFeeRefund =
      withdrawal.withdrawals && Array.isArray(withdrawal.withdrawals)
        ? withdrawal.withdrawals.reduce((acc, w) => acc + BigInt(w.storageFeeRefund), BigInt(0))
        : BigInt(0)
    ...
    if (withdrawal.withdrawalInShares) {
      nominator.pendingShares = withdrawal.withdrawalInShares.shares ?? BigInt(0)
