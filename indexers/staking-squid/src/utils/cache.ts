...
export type LastBlockBundleIndexKey = `lastBlockBundleIndex:${string}-${string}`
export type AllTimeHighStakedKey = `allTimeHighStaked:${string}`
export type AllTimeHighSharePriceKey = `allTimeHighSharePrice:${string}:${string}`
type InternalKeyStore = LastBlockBundleIndexKey | AllTimeHighStakedKey | AllTimeHighSharePriceKey
...
export const load = async (ctx: Ctx<Store>): Promise<Cache> => {
  const [domains, accounts, operators, nominators, deposits, withdrawals, domainEpochs] =
    await Promise.all([
