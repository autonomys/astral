...
export function processOperatorRewardedEvent(cache: Cache, block: CtxBlock, event: CtxEvent) {
  const operatorId = String(event.args.operatorId) ?? '0'

  const reward = BigInt(event.args.reward)
  if (reward === BigInt(0)) return cache

  const operatorTotalRewardsCollected = getOrCreateOperatorTotalRewardsCollected(
    cache,
    block,
    operatorId,
  )
...
export function processOperatorTaxCollectedEvent(cache: Cache, block: CtxBlock, event: CtxEvent) {
  const operatorId = String(event.args.operatorId) ?? '0'
  const tax = BigInt(event.args.tax ?? 0)

  const operatorTotalTaxCollected = getOrCreateOperatorTotalTaxCollected(cache, block, operatorId)
  ...
  cache.operatorTotalTaxCollected.set(operatorTotalTaxCollected.id, operatorTotalTaxCollected)
...
export function processBundleStoredEvent(cache: Cache, block: CtxBlock, event: CtxEvent) {
  const operatorId = String(event.args.bundleAuthor) ?? '0'

  const operatorBundleTotalCount = getOrCreateOperatorBundleTotalCount(cache, block, operatorId)
  ...
  cache.operatorBundleTotalCount.set(operatorBundleTotalCount.id, operatorBundleTotalCount)
...
export function processOperatorRegisteredEvent(cache: Cache, block: CtxBlock, event: CtxEvent) {
  const operatorId = String(event.args.operatorId) ?? '0'

  const operatorDepositsTotalCount = getOrCreateOperatorDepositsTotalCount(cache, block, operatorId)
...
export function processOperatorNominatedEvent(cache: Cache, block: CtxBlock, event: CtxEvent) {
  const operatorId = String(event.args.operatorId) ?? '0'
  ...
  const operatorDepositsTotalValue = getOrCreateOperatorDepositsTotalValue(cache, block, operatorId)
  ...
  cache.operatorDepositsTotalValue.set(operatorDepositsTotalValue.id, operatorDepositsTotalValue)
  ...
  cache.nominatorDepositsTotalCount.set(nominatorDepositsTotalCount.id, nominatorDepositsTotalCount)
  ...
  cache.nominatorDepositsTotalValue.set(nominatorDepositsTotalValue.id, nominatorDepositsTotalValue)
...
export function processWithdrewStakeEvent(cache: Cache, block: CtxBlock, event: CtxEvent) {
  const operatorId = String(event.args.operatorId) ?? '0'
  const accountId = hexToAccount(event.args.nominatorId)

  const operatorWithdrawalsTotalCount = getOrCreateOperatorWithdrawalsTotalCount(
    cache,
    block,
    operatorId,
  )
  ...
  const nominatorWithdrawalsTotalCount = getOrCreateNominatorWithdrawalsTotalCount(
    cache,
    block,
    accountId,
  )
