...
export function processTransactionFeePaidEvent(cache: Cache, block: CtxBlock, event: CtxEvent) {
  ...
  const accountTransactionFeePaidTotalValue = getOrCreateAccountTransactionFeePaidTotalValue(
    cache,
    block,
    accountId,
  )
