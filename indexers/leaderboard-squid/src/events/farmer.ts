...
export function processFarmerVoteRewardEvent(cache: Cache, block: CtxBlock, event: CtxEvent) {
  const accountId = hexToAccount(event.args.voter)
  const reward = BigInt(event.args.reward)

  const farmerVoteTotalCount = getOrCreateFarmerVoteTotalCount(cache, block, accountId)
  ...
  const farmerVoteTotalValue = getOrCreateFarmerVoteTotalValue(cache, block, accountId)
...
export function processFarmerBlockRewardEvent(cache: Cache, block: CtxBlock, event: CtxEvent) {
  const accountId = hexToAccount(event.args.blockAuthor)
  const reward = BigInt(event.args.reward)

  const farmerBlockTotalCount = getOrCreateFarmerBlockTotalCount(cache, block, accountId)
  ...
  cache.farmerBlockTotalCount.set(farmerBlockTotalCount.id, farmerBlockTotalCount)

  const farmerBlockTotalValue = getOrCreateFarmerBlockTotalValue(cache, block, accountId)
  ...
  cache.farmerBlockTotalValue.set(farmerBlockTotalValue.id, farmerBlockTotalValue)
