...
import { processExtrinsicFailedEvent, processExtrinsicSuccessEvent } from './extrinsic' // Import the new processing functions
import { processFarmerBlockRewardEvent, processFarmerVoteRewardEvent } from './farmer'
...
async function processEvent(cache: Cache, api: ApiPromise, block: CtxBlock, event: CtxEvent) {
  switch (event.name) {
    // account events
