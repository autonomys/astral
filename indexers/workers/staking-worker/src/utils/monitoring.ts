...
export class PerformanceMonitor {
   ...
   */
  getStats(): { [taskType: string]: { totalProcessed: number; avgTimePerTask: number; successRate: number } } {
    const stats: { [taskType: string]: { totalProcessed: number; avgTimePerTask: number; successRate: number } } = {};
    ...
    for (const metric of this.metrics.values()) {
      ...
      taskStats.avgTimePerTask = 
        (taskStats.avgTimePerTask * prevTotal + (metric.averageTimePerTask || 0) * metric.totalTasks) / 
        taskStats.totalProcessed;
      taskStats.successRate = 
        (taskStats.successRate * prevTotal + (metric.successfulTasks / metric.totalTasks) * metric.totalTasks) / 
        taskStats.totalProcessed;
    }
