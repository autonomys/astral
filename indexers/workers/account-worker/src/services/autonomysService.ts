...
const connectAutonomysApi = async (): Promise<ApiPromise> => {
  ...
  try {
    ...
    provider.on('disconnected', () =>
      console.warn(
        'WsProvider for worker disconnected. Attempting to reconnect implicitly if ApiPromise is used.',
      ),
    );
    ...
    console.log(
      `Worker API - Chain: ${workerApiPromise.runtimeChain}, Version: ${workerApiPromise.runtimeVersion}`,
    );
    return workerApiPromise;
  } catch (error) {
...
const getMultipleAccountsDataAtBlock = async (
  ...
  try {
    ...
    console.log(
      `Worker: Successfully fetched ${accountResults.filter((r) => r.data).length}/${addresses.length} accounts`,
    );
    return results;
  } catch (error) {
    ...
    if (
      ...
      error.message &&
      error.message.includes('State already discarded')
    ) {
